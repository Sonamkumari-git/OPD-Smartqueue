...
export type Vitals = { id: string; patient_id: string; patient_name?: string | null; token_id: string; recorded_by: string; temperature: number; heart_rate: number; blood_pressure: { systolic: number; diastolic: number }; spo2: number; recorded_at: string; updated_at?: string | null };
...
export const api = {
  ...
  updateVitals: (id: string, payload: { temperature: number; heart_rate: number; blood_pressure: { systolic: number; diastolic: number }; spo2: number }, token: string) => request<Vitals>(`/api/vitals/${id}`, { method: "PATCH", body: JSON.stringify(payload) }, token),
  deleteVitals: (id: string, token: string) => request<{ id: string }>(`/api/vitals/${id}`, { method: "DELETE" }, token),
