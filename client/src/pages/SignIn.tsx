    ...
    <div className="min-h-screen bg-[#F6F4EE] text-[#15334A]">
      <div className="grid min-h-screen lg:grid-cols-[0.9fr_1.1fr]">
        <aside className="relative hidden overflow-hidden bg-[#15334A] p-12 text-white lg:flex lg:flex-col">
          ...
          <div className="relative flex items-center gap-3"><img src="/queue-assets/queue-pulse-logo.webp" alt="OPD SmartQueue logo" className="h-12 w-12" /><div><p className="text-[9px] font-extrabold uppercase tracking-[0.28em] text-[#76D6CB]">OPD / LIVE CARE RAIL</p><p className="mt-1 font-[Fraunces] text-xl font-semibold tracking-[-0.055em]">SmartQueue</p></div></div>
          <div className="relative my-auto max-w-md"><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#76D6CB]">{selected ? `${selected.label} access / scoped session` : "Choose your workspace"}</p><h1 className="mt-5 font-[Fraunces] text-5xl font-semibold leading-tight tracking-[-0.05em]">{selected ? `Sign in to the ${selected.label.toLowerCase()} workspace.` : "One secure door for every care role."}</h1><p className="mt-6 leading-7 text-white/65">Each workspace is authenticated against the live backend and displays only the operational data permitted for that role.</p></div>
          <svg viewBox="0 0 520 120" fill="none" aria-hidden="true" className="pointer-events-none absolute bottom-20 left-0 w-[42rem] max-w-none text-[#76D6CB]/35"><path d="M-20 86c90 16 105-48 184-30 77 18 105-22 175-35 76-14 88 50 200 8" stroke="currentColor" strokeWidth="2" /><circle cx="164" cy="56" r="7" fill="currentColor" /><circle cx="339" cy="21" r="7" fill="currentColor" /><circle cx="487" cy="59" r="7" fill="currentColor" /></svg>
          <div className="relative flex items-center gap-3 border-t border-white/15 pt-6 text-xs font-bold text-white/65"><ShieldCheck className="h-4 w-4 text-[#76D6CB]" /> Session boundary active — no preview or synthetic access.</div>
        </aside>
          ...
          <div className="w-full max-w-xl">
            ...
            {selected ? (
              <div className="rounded-[2rem] border border-[#15334A]/10 bg-white p-6 shadow-[0_20px_55px_rgba(21,51,74,0.09)] sm:p-9">
                <div className="flex items-start justify-between gap-5"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.17em] text-[#0F8F83]">{selected.label} sign in / live identity</p><h2 className="mt-3 font-[Fraunces] text-4xl font-semibold tracking-[-0.05em]">Open your role workspace.</h2></div><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#E0F3EF] text-[#0F8F83]"><SelectedIcon className="h-5 w-5" /></div></div>
                <div className="mt-4 flex items-center gap-2 text-[#0F8F83]"><span className="h-px w-12 bg-current" /><span className="h-2 w-2 rounded-full bg-current" /><span className="h-px flex-1 bg-current/20" /></div>
